...
int is_dup_ack(flows_t *flow, struct my_tcp_custom_hdr *tcp)
{
	acks_t *p = flow->acks;
	int count = 0;

	while (p) {
		if (p->ack == ntohl(tcp->th_ack)) 
			count++;
		p = p->next;
	}
	if (count <= 1)
...
void calculate_rtt(flows_t *f, struct timeval end, struct my_tcp_custom_hdr *tcp)
{
	...
	while(s) {
		if (s->exp_ack == ntohl(tcp->th_ack) && !is_dup_ack(f, tcp)) {
			start.tv_sec = s->sec;
			...
			unsigned int capture_len)
{
	...
	tcp = (struct my_tcp_custom_hdr*) packet;
		...
		flow->acks = NULL;
		flow->frt = 0;
		flow->mss = get_mss((char *)packet+sizeof(struct my_tcp_custom_hdr));
		flow->cwind.idx = 0;
		put_seq(flow, seq);
		flow->next = NULL;
...
void print_part1(flow_context_t *f)
{
	...
	while (fs) {
		...
		timeval_subtract(&r, &end, &start);
		printf("\tEmpirical Throughput: %fMbps\n",((float)(fs->tx_bytes-fs->retransmissions)*8/(float)(r.tv_sec*1000000+r.tv_usec)));
		printf("\tLossrate: %f\n",(float)(fs->retransmission_pkts)/(float)(fs->tx_pkts));
		calculate_avg_rtt(fs);
		printf("\tRTT: %f\n",fs->rtt);
		if ((float)(fs->retransmission_pkts)/(float)(fs->tx_pkts) > 0) {
			th = (sqrt(3/2)*fs->mss*8)/(fs->rtt*sqrt((float)(fs->retransmission_pkts)/(float)(fs->tx_pkts)));
			printf("\tTheoretical Throughput: %fMbps\n\n",th);
		} else
			printf("\tTheoretical Throughput: Infinity\n\n");
		...
		//printf("\n");
		//c = 0;
		//printf("\t Number of Fast Retransmits: %d\n",fs->frt);
